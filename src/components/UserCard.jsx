export default function UserCard({ user = {}, distance, onView }) {
  ...
  return (
    <article className="rounded-2xl border border-amber-100 bg-white p-5 shadow-sm transition-shadow duration-200 hover:shadow-md">
      <div className="flex items-center gap-4">
        <div aria-hidden="true" className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-amber-100 text-xl font-bold text-amber-800">
          {initials}
        </div>
        ...
        {distance && (
          <div className="text-right shrink-0" aria-label={`${distance} away`}>
            <p className="text-base font-semibold text-amber-500" aria-hidden="true">{distance}</p>
            <p className="text-xs text-slate-400" aria-hidden="true">away</p>
          </div>
        )}
      </div>
