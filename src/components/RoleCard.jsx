export default function RoleCard({ title, description, role, selected, onSelect }) {
  return (
    ...
    >
      <div className="text-xs uppercase tracking-[0.4em] text-amber-500" aria-hidden="true">{role}</div>
      <h3 className="mt-2 text-2xl text-slate-900">{title}</h3>
      <p className="mt-3 text-sm leading-relaxed text-slate-500">{description}</p>
      {selected && (
        <div aria-hidden="true" className="mt-4 inline-block rounded-full bg-amber-500 px-3 py-1 text-xs font-semibold text-white">Selected</div>
      )}
    </button>
  );
}
